...
PATH_PROJECT = $(DESTDIR)/var/www.rpg/$(INSTALL_DIR)

.PHONY: help install update build dchv dchr
...
install: 	##@system Install package. Don't run it manually!!!
	...
	@sed -i 's/<meta name="version" content="VERSION_PLACEHOLDER">/<meta name="version" content="Version $(VERSION) from $(DATE), build hash $(COMMIT_HASH)">/' $(PATH_PROJECT)/index.html
